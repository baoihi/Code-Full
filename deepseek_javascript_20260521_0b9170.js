...
// @version 4.2
// @description Menu điều khiển - Tự động đăng nhập từ userDropdown
// @author YourName
// @match *://*/*
...
(function() {
    ...
    function getUsernameFromPage() {
        ...
        if (userDropdown) {
            ...
            // Loại bỏ icon và khoảng trắng, lấy username
        ...
        // Cách 3: Tìm bất kỳ a nào có class chứa dropdown-toggle và có icon user-circle
        const anyUserBtn = document.querySelector('a[class*="dropdown-toggle"] .fa-user-circle, a[class*="dropdown-toggle"] .fas.fa-user-circle');
        if (anyUserBtn) {
            const parent = anyUserBtn.closest('a');
            if (parent) {
                const text = parent.textContent || parent.innerText;
                const username = text.replace(/<i[^>]*><\/i>/, '').replace(/[\n\t]/g, '').trim();
                if (username) {
                    console.log('[Auto-Login] Đã tìm thấy username từ dropdown-toggle:', username);
                    return username;
                }
            }
        }
    ...
    fab.onclick = function() {
        if (panel.style.display === 'none' || panel.style.display === '') {
            ...
            if (savedLogin === 'true') {
                ...
                // Thử lấy username từ trang
                const pageUsername = getUsernameFromPage();
                if (pageUsername) {
                    // Có username, tự động đăng nhập
    ...
    async function init() {
        ...
        if (savedLogin === 'true') {
            ...
            // Thử lấy username từ trang hiện tại
            const pageUsername = getUsernameFromPage();
            if (pageUsername) {
                doLogin(pageUsername);
            }
        ...
        console.log('%c🎯 Control Panel v4.2 - Tự động đăng nhập từ userDropdown!', 'color: #0f0; font-size: 14px');
        console.log('%c👆 Click vào nút tròn để mở menu', 'color: #ffd700;');
        console.log('%c🔑 Đăng nhập tự động khi có a#userDropdown', 'color: #00adb5;');
