...
// @version      3.3
// @description  Menu điều khiển với Auto Clicker, Music và SpeedHack (cần đăng nhập để dùng)
// @author       YourName
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function() {
    ...
    function showLoginModal(callback) {
        ...
        // Xử lý đăng nhập - NHẬP GÌ CŨNG ĐƯỢC
        overlay.querySelector('#login-confirm-btn').onclick = () => {
            ...
            // ✅ NHẬP GÌ CŨNG ĐƯỢC - luôn thành công
            isLoggedIn = true;
    ...
    function requireLogin(callback) {
        // Kiểm tra đã đăng nhập chưa (lưu trong localStorage)
    ...
    function showVIPModal(callback) {
        ...
        // Kiểm tra key đã lưu còn hạn không
    ...
    function createSpeedHackSubmenu() {
        ...
        if (toggleBtn) {
            toggleBtn.onclick = async (e) => {
                e.stopPropagation();
                // Yêu cầu đăng nhập trước khi bật speedhack
                requireLogin(async () => {
    ...
    function createAutoClickerSubmenu() {
        ...
        if (toggleBtn) {
            toggleBtn.onclick = async (e) => {
                e.stopPropagation();
                // Yêu cầu đăng nhập trước khi bật auto clicker
                requireLogin(async () => {
    ...
    function createMusicSubmenu() {
        ...
        if (toggleMuteGame) {
            toggleMuteGame.onclick = (e) => {
                e.stopPropagation();
                // Yêu cầu đăng nhập trước khi tắt âm game
                requireLogin(() => {
        ...
        if (playBtn) {
            playBtn.onclick = (e) => {
                e.stopPropagation();
                requireLogin(() => playMusic());
            };
        }
        if (stopBtn) {
            stopBtn.onclick = (e) => {
                e.stopPropagation();
                requireLogin(() => stopMusic());
            };
        }
    ...
    // ========== TẠO SUBMENU VIP (CHỈ MỤC NÀO CÓ VIP BADGE MỚI CẦN KEY) ==========
    function createVipSubmenu() {
        ...
        // Toggle Log Iframe - KHÔNG cần VIP nhưng cần đăng nhập
        ...
        // Toggle Log Account - CÓ VIP BADGE -> CẦN KEY + đăng nhập
        ...
        if (toggleLogAccount) {
            toggleLogAccount.onclick = (e) => {
                ...
                requireLogin(() => {
                    // KIỂM TRA VIP TRƯỚC KHI BẬT
                    if (!isVIPActive) {
                        showVIPModal((success) => {
                            if (success) {
                                // Kích hoạt toggle sau khi VIP được xác nhận
            ...
            activateBtn.onclick = (e) => {
                ...
                requireLogin(() => {
                    showVIPModal((success) => {
                        if (success) {
                            updateVIPBadge();
                            createVipSubmenu(); // Refresh submenu
                        }
                    });
                });
            };
    ...
    function setupSubmenu(itemId, arrowId, submenuId, createFn, expandedFlag) {
        ...
        if (item) {
            item.addEventListener('click', (e) => {
                e.stopPropagation();
                // Yêu cầu đăng nhập trước khi mở rộng bất kỳ submenu nào
                requireLogin(() => {
                    if (itemId === 'vipItem') {
                        if (vipExpanded) {
                    ...
                    } else if (itemId === 'autoClickerItem') {
                        if (autoClickerExpanded) {
                            arrow.classList.remove('expanded');
                            submenu.classList.remove('show');
                            autoClickerExpanded = false;
                        } else {
                            arrow.classList.add('expanded');
                            submenu.classList.add('show');
                            createFn();
                            autoClickerExpanded = true;
                        }
                    } else if (itemId === 'speedHackItem') {
                        if (speedHackExpanded) {
                            arrow.classList.remove('expanded');
                            submenu.classList.remove('show');
                            speedHackExpanded = false;
                        } else {
                            arrow.classList.add('expanded');
                            submenu.classList.add('show');
                            createFn();
                            speedHackExpanded = true;
                        }
                    } else if (itemId === 'musicItem') {
                        if (musicExpanded) {
                            arrow.classList.remove('expanded');
                            submenu.classList.remove('show');
                            musicExpanded = false;
    ...
    setupSubmenu('autoClickerItem', 'autoClickerArrow', 'autoClickerSubmenu', createAutoClickerSubmenu, null);
    setupSubmenu('speedHackItem', 'speedHackArrow', 'speedHackSubmenu', createSpeedHackSubmenu, null);
    setupSubmenu('musicItem', 'musicArrow', 'musicSubmenu', createMusicSubmenu, null);
    ...
    async function init() {
        ...
        console.log('%c✅ Control Panel v3.3 đã sẵn sàng!', 'color: #0f0; font-size: 14px');
        console.log('%c🔐 Click vào menu để đăng nhập và sử dụng tính năng', 'color: #00adb5;');
    }
