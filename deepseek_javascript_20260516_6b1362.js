...
// @version 3.7
// @description Menu điều khiển với Auto Clicker, Music và SpeedHack (VIP cần key) - Tự động đăng nhập theo username từ trang
// @author YourName
// @match *://*/*
...
(function() {
    ...
    function getUsernameFromPage() {
        // Tìm username trong thẻ h3 có class "rs"
        const usernameElement = document.querySelector('h3.rs');
        if (usernameElement && usernameElement.textContent.trim()) {
            const username = usernameElement.textContent.trim();
            console.log('[Auto-Login] Đã tìm thấy username từ h3.rs:', username);
            return username;
        }
        
        // Fallback: tìm trong input disabled (trường hợp dự phòng)
        const disabledInput = document.querySelector('input.input-code[disabled][value]');
        if (disabledInput && disabledInput.value) {
            const username = disabledInput.value.trim();
            console.log('[Auto-Login] Đã tìm thấy username từ input disabled:', username);
            return username;
        }
        
        console.log('[Auto-Login] Không tìm thấy username trên trang');
        return null;
    }
    ...
    function autoLogin(username, callback) {
        if (!username) {
            console.log('[Auto-Login] Không có username để đăng nhập');
        ...
        console.log('[Auto-Login] Đã tự động đăng nhập với tài khoản:', username);
        
        if (typeof GM_notification === 'function') {
            GM_notification({ text: 'Chào mừng ' + username + '! (Tự động đăng nhập)', title: 'Đăng nhập thành công', timeout: 2000 });
        } else {
    ...
    // ========== MODAL ĐĂNG NHẬP (CHỈ HIỆN KHI KHÔNG TỰ ĐỘNG ĐƯỢC) ==========
    function showLoginModal(callback) {
        if (vipModalVisible) return;
        
        // Kiểm tra xem đã có username từ trang chưa
        const pageUsername = getUsernameFromPage();
        if (pageUsername) {
            // Tự động đăng nhập, không cần hiện modal
            autoLogin(pageUsername, callback);
            return;
        }
        
        // Nếu không có username trên trang thì mới hiện modal đăng nhập thủ công
        ...
        overlay.innerHTML = '<div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border: 2px solid #667eea; border-radius: 16px; padding: 25px; width: 380px; color: white; font-family: \'Segoe UI\', Arial, sans-serif; text-align: center;">' +
            ...
            '<p style="color: #ccc; font-size: 13px; margin-bottom: 20px;">Không tìm thấy thông tin tài khoản trên trang. Vui lòng đăng nhập thủ công.</p>' +
    ...
    fab.onclick = function() {
        if (panel.style.display === 'none' || panel.style.display === '') {
            ...
            if (!isLoggedIn) {
                // Gọi showLoginModal - hàm này sẽ tự động kiểm tra và đăng nhập nếu có username từ trang
    ...
    async function init() {
        ...
        if (savedLogin === 'true') {
            ...
            // Thử lấy username từ trang và tự động đăng nhập
            const pageUsername = getUsernameFromPage();
            if (pageUsername) {
                console.log('[Auto-Login] Phát hiện username trên trang:', pageUsername);
                autoLogin(pageUsername, function(success) {
                    if (success) {
                        updateMenuAfterLogin();
                    }
                });
        ...
        console.log('%c🎯 Control Panel v3.7 (Tự động đăng nhập theo username từ trang)!', 'color: #0f0; font-size: 14px');
        console.log('%c👆 Click vào nút tròn để mở menu', 'color: #ffd700;');
        console.log('%c🔑 Tự động đăng nhập với username từ thẻ <h3 class="rs">', 'color: #00adb5;');
        console.log('%c🎵 Playlist: 6 bài hát (5 Bùi Trường Linh + Thunder)', 'color: #4caf50;');
    }
