...
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/576174/Control%20Pal%20Auto%20Clicker%20%20Music%20%20SpeedHack.user.js
// @updateURL https://update.greasyfork.org/scripts/576174/Control%20Pal%20Auto%20Clicker%20%20Music%20%20SpeedHack.meta.js
// ==/UserScript==
...
(function() {
    ...
    function showLoginModal(callback) {
        ...
        overlay.innerHTML = '<div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border: 2px solid #667eea; border-radius: 16px; padding: 25px; width: 380px; color: white; font-family: \'Segoe UI\', Arial, sans-serif; text-align: center;">' +
            '<div style="font-size: 50px; margin-bottom: 10px;">🔐</div>' +
            '<h2 style="color: #667eea; margin: 0 0 8px 0;">ĐĂNG NHẬP</h2>' +
            '<p style="color: #ccc; font-size: 13px; margin-bottom: 20px;">Vui lòng đăng nhập để sử dụng Control Panel</p>' +
            '<input type="text" id="login-username" placeholder="Tên đăng nhập (tối đa 16 ký tự)..." maxlength="16" style="width: 100%; padding: 12px; border: 2px solid #444; border-radius: 10px; background: rgba(255,255,255,0.05); color: white; font-size: 14px; box-sizing: border-box; outline: none; margin-bottom: 12px;" autocomplete="off">' +
            '<input type="password" id="login-password" placeholder="Mật khẩu..." style="width: 100%; padding: 12px; border: 2px solid #444; border-radius: 10px; background: rgba(255,255,255,0.05); color: white; font-size: 14px; box-sizing: border-box; outline: none; margin-bottom: 10px;" autocomplete="off">' +
            '<div id="login-error" style="color: #ff6b6b; font-size: 12px; min-height: 18px; margin-bottom: 15px;"></div>' +
            '<div style="display: flex; gap: 10px;">' +
            '<button id="login-confirm-btn" style="flex: 1; padding: 12px; border: none; border-radius: 10px; background: linear-gradient(135deg, #667eea, #764ba2); color: white; font-weight: bold; font-size: 15px; cursor: pointer;">Đăng Nhập</button>' +
            '<button id="login-cancel-btn" style="flex: 1; padding: 12px; border: 1px solid #555; border-radius: 10px; background: transparent; color: #aaa; font-size: 15px; cursor: pointer;">Bỏ Qua</button>' +
            '</div>' +
            '<div style="margin-top: 12px; font-size: 11px; color: #888;">Nhập username và password bất kỳ đều được</div>' +
            '</div>';
        ...
        overlay.querySelector('#login-confirm-btn').onclick = function() {
            ...
            if (username.length > 16) {
                errorDiv.textContent = '⚠️ Tên đăng nhập tối đa 16 ký tự!';
                return;
            }
        ...
        overlay.querySelector('#login-cancel-btn').onclick = function() {
            overlay.remove();
            vipModalVisible = false;
    ...
    function updateMenuAfterLogin() {
        ...
        content.innerHTML = '<div style="background:rgba(0,0,0,0.3); padding:8px 12px; border-radius:10px; margin-bottom:10px; text-align:center; border:1px solid #667eea;">Tài khoản: <span style="color:#667eea; font-weight:bold;">' + username + '</span> ' + statusBadge + '</div>' +
    ...
    document.querySelectorAll('.main-menu-fab, .main-menu-panel, #auto-clicker-panel, .autoClick-fab, .autoClick-panel, .autoClick-menu, #music-round-btn, #music-panel, .uac-gui-container, .uac-floating-btn, #speedhack-ui, #speedhack-toggle-btn, #floatingNotification, #vip-modal-overlay, #login-modal-overlay').forEach(function(el) {
    ...
    addStyle('.main-menu-fab{position:fixed;bottom:80px;right:20px;width:55px;height:55px;border-radius:50%;background:#00adb5;box-shadow:0 4px 15px rgba(0,0,0,0.3);z-index:999999;cursor:grab;overflow:hidden;border:2px solid white;display:flex;align-items:center;justify-content:center;touch-action:none;}' +
        ...
        '.submenu-container.show{max-height:800px;}' +
        ...
        '.music-category{color:#667eea;font-weight:bold;font-size:12px;margin-bottom:5px;padding:5px 8px;background:rgba(102,126,234,0.15);border-radius:5px;border-left:2px solid #667eea;}' +
    ...
    // Playlist chia thành 2 mục
    let musicCategories = [
        {
            name: 'Nhạc Bùi Trường Linh',
            songs: [
        ...
        },
        {
            name: 'POV: 2018',
            songs: [
                { name: 'POV: 2018', url: 'https://files.catbox.moe/gazicu.mp3', artist: '' }
            ]
        }
    ];
    
    // Lưu index của category và bài hát hiện tại
    ...
    function playMusic(categoryIndex, songIndex) {
        try {
            if (categoryIndex === undefined || categoryIndex === null) categoryIndex = currentCategoryIndex;
            if (songIndex === undefined || songIndex === null) songIndex = currentSongIndex;
            
            const category = musicCategories[categoryIndex];
            if (!category || !category.songs[songIndex]) {
                console.error("Không tìm thấy bài hát!");
                return;
            }
            
            if (myAudio && isPlaying && categoryIndex === currentCategoryIndex && songIndex === currentSongIndex && !myAudio.paused) return;
            ...
            myAudio = new Audio();
            myAudio.src = category.songs[songIndex].url;
            myAudio.loop = true;
            myAudio.volume = 0.8;
            myAudio.muted = false;
    ...
    function updateMusicStatus() {
        ...
        if (statusDiv) {
            if (isPlaying) {
                const category = musicCategories[currentCategoryIndex];
                const song = category.songs[currentSongIndex];
                if (song.artist) {
    ...
    function createMusicSubmenu() {
        ...
        for (let catIndex = 0; catIndex < musicCategories.length; catIndex++) {
            const category = musicCategories[catIndex];
            let songOptions = '';
            for (let songIndex = 0; songIndex < category.songs.length; songIndex++) {
                const song = category.songs[songIndex];
                const selected = (catIndex === currentCategoryIndex && songIndex === currentSongIndex) ? ' selected' : '';
                const displayText = song.artist ? song.name + ' - ' + song.artist : song.name;
                songOptions += '<option value="' + catIndex + '-' + songIndex + '"' + selected + '>' + displayText + '</option>';
            }
            
            musicHTML += '<div class="music-category">🎵 ' + category.name + '</div>';
            musicHTML += '<div class="submenu-content" style="flex-direction: column; align-items: stretch; margin-bottom: 8px;">';
            musicHTML += '<select class="music-select" data-category="' + catIndex + '" style="width: 100%; padding: 8px; background: rgba(255,255,255,0.1); color: white; border: 1px solid #667eea; border-radius: 8px; font-size: 12px; outline: none; cursor: pointer; font-family: \'Segoe UI\', Arial, sans-serif; margin-top: 5px;">';
            musicHTML += songOptions;
            musicHTML += '</select></div>';
        }
        
        let statusText = '❌ Chưa phát';
        if (isPlaying) {
            const cat = musicCategories[currentCategoryIndex];
            const song = cat.songs[currentSongIndex];
        ...
        document.getElementById('playMusicBtn').onclick = function(e) {
            e.stopPropagation();
            const allSelects = document.querySelectorAll('.music-select');
            let selectedValue = null;
            for (let i = 0; i < allSelects.length; i++) {
                if (parseInt(allSelects[i].dataset.category) === currentCategoryIndex) {
                    selectedValue = allSelects[i].value;
                    break;
                }
            }
            if (!selectedValue && allSelects.length > 0) {
                selectedValue = allSelects[0].value;
            }
            if (selectedValue) {
                const parts = selectedValue.split('-');
                const catIndex = parseInt(parts[0]);
                const songIndex = parseInt(parts[1]);
                playMusic(catIndex, songIndex);
            }
        };
        ...
        document.querySelectorAll('.music-select').forEach(function(select) {
    ...
    fab.onclick = function() {
        if (panel.style.display === 'none' || panel.style.display === '') {
            ...
            if (!isLoggedIn) {
                showLoginModal(function(success) {
                    if (success) {
                        updateMenuAfterLogin();
                    }
                });
    ...
    async function init() {
        ...
        console.log('%c🎵 Playlist: 5 bài Bùi Trường Linh + POV 2018', 'color: #4caf50;');
    }
