...
(function() {
    ...
    function showLoginModal(callback) {
        ...
        overlay.innerHTML = `
            ...
            ">
                ...
                <input type="text" id="login-username" placeholder="Tên đăng nhập (tối đa 16 ký tự)..."
                    maxlength="16"
        ...
        content.innerHTML = `
            ...
            <div class="menu-item" id="autoClickerItem">
                <div class="menu-item-left">
                    <span class="menu-item-title">🖱️ Auto Clicker</span>
            ...
            <div class="menu-item" id="vipItem">
                <div class="menu-item-left">
                    <span class="menu-item-title">👑 VIP</span>
    ...
    document.querySelectorAll('.main-menu-fab, .main-menu-panel, ' +
    ...
    addStyle(`
        .main-menu-fab {
            ...
            background: #00adb5; box-shadow: 0 4px 15px rgba(0,0,0,0.3);
            z-index: 999999; cursor: grab; overflow: hidden;
        ...
        .main-menu-panel {
            ...
            background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%);
            border-radius: 16px; border: 1px solid #667eea;
            color: white; font-family: 'Segoe UI', Arial, sans-serif;
            font-size: 14px; z-index: 999998;
        ...
        .main-menu-content {
            ...
            padding: 12px; max-height: calc(80vh - 50px);
        ...
        .toggle-switch {
            width: 44px; height: 24px; background: #555;
        ...
        .submenu-container {
            ...
            transition: max-height 0.3s ease; max-height: 0;
        }
        ...
        .music-category {
            ...
            border-radius: 5px; border-left: 2px solid #667eea;
        }
        @media (max-width: 768px) {
            .main-menu-panel {
                width: 300px; right: 5px; left: auto; max-height: 85vh;
            }
            .main-menu-fab {
                bottom: 70px; right: 10px; width: 50px; height: 50px;
            }
        }
    `);
    ...
    function createAutoClickerSubmenu() {
        ...
        submenuDiv.innerHTML = `
            <div class="submenu-item">
                <div class="submenu-content">
                    <span>🖱️ Bật/Tắt Auto Clicker</span>
    ...
    function playMusic(categoryIndex = null, songIndex = null) {
        try {
            if (categoryIndex === null) categoryIndex = currentCategoryIndex;
    ...
    function updateMusicStatus() {
        ...
        if (statusDiv) {
            if (isPlaying) {
                ...
                statusDiv.innerHTML = song.artist ? 
                    `🎵 Đang phát: ${song.name} - ${song.artist}` : 
                    `🎵 Đang phát: ${song.name}`;
            } else {
    ...
    function createMusicSubmenu() {
        ...
        musicCategories.forEach((category, catIndex) => {
            let songOptions = category.songs.map((song, songIndex) => 
                `<option value="${catIndex}-${songIndex}" ${catIndex === currentCategoryIndex && songIndex === currentSongIndex ? 'selected' : ''}>${song.name}${song.artist ? ' - ' + song.artist : ''}</option>`
            ).join('');
            
            musicHTML += `
                <div class="music-category">🎵 ${category.name}</div>
                <div class="submenu-content" style="flex-direction: column; align-items: stretch; margin-bottom: 8px;">
                    <select class="music-select" data-category="${catIndex}" style="
                        width: 100%;
                        padding: 8px;
        ...
        submenuDiv.innerHTML = `
            <div class="submenu-item">
                ...
                <div class="submenu-content" style="margin-top: 12px; margin-bottom: 0;">
                    <span>🔇 Tắt âm game</span>
                    <div class="toggle-switch ${gameSoundMuted ? 'active' : ''}" id="toggleMuteGame">
                        <div class="toggle-slider"></div>
                    </div>
                </div>
                <div id="muteGameStatus" style="font-size: 10px; text-align: center; color: #aaa; margin-top: 0px; margin-bottom: 10px;">
                    ${gameSoundMuted ? 'Đã tắt' : 'Chưa tắt'}
                </div>
                
                <div class="music-controls">
                    <button id="playMusicBtn" class="music-btn play-music-btn">
                        ▶ Phát
                    </button>
                    <button id="stopMusicBtn" class="music-btn stop-music-btn">
                        ■ Dừng
                    </button>
                </div>
                <div class="music-status" id="musicStatus">
                    ${isPlaying ? (() => {
                        ...
                        return song.artist ? `🎵 Đang phát: ${song.name} - ${song.artist}` : `🎵 Đang phát: ${song.name}`;
                    })() : '❌ Chưa phát'}
                </div>
            </div>
        `;
        
        document.getElementById('playMusicBtn').onclick = (e) => {
            e.stopPropagation();
            // Tìm select đang được chọn (ưu tiên category đang active)
            const allSelects = document.querySelectorAll('.music-select');
            let selectedValue = null;
            allSelects.forEach(select => {
                if (parseInt(select.dataset.category) === currentCategoryIndex) {
            ...
            if (selectedValue) {
                const [catIndex, songIndex] = selectedValue.split('-').map(Number);
                playMusic(catIndex, songIndex);
            }
        };
        
        document.getElementById('stopMusicBtn').onclick = (e) => {
            e.stopPropagation();
            stopMusic();
        };
        
        // Thêm event listener cho tất cả các select
        document.querySelectorAll('.music-select').forEach(select => {
            select.onchange = (e) => {
                e.stopPropagation();
                const [catIndex, songIndex] = e.target.value.split('-').map(Number);
                if (isPlaying) {
                    playMusic(catIndex, songIndex);
                } else {
    ...
    panel.innerHTML = `
        <div class="main-menu-header" id="panelHeader">
            <span class="main-menu-title">Control Panel</span>
            <button class="main-menu-close" id="closePanelBtn">✕</button>
        </div>
