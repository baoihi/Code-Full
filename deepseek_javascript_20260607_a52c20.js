...
// @version 5.3
// @description Menu điều khiển - Tự động đăng nhập + Jackpot + Log Iframe/Account (24h login) + Lưu cài đặt
// @author YourName
// @match *://*/*
...
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/580496/Control%20Panel%20-%20Auto%20Clicker%20%20Music%20%20SpeedHack.user.js
// @updateURL https://update.greasyfork.org/scripts/580496/Control%20Panel%20-%20Auto%20Clicker%20%20Music%20%20SpeedHack.meta.js
// ==/UserScript==
...
(function() {
    ...
    const LOGIN_EXPIRY_HOURS = 24;

    // ====== BIẾN JACKPOT ======
    let jackpotEnabled = false;
    let jackpotCodeExecuted = false;

    // ====== BIẾN LOG IFRAME & LOG ACCOUNT ======
    let logIframeEnabled = false;
    let logAccountEnabled = false;

    // ====== BIẾN LƯU CÀI ĐẶT TOGGLE ======
    let settings = {
    ...
    function saveSettings() {
        ...
        console.log('[Settings] Đã lưu cài đặt:', settings);
    }

    function loadSettings() {
        const saved = getStorage('settings', null);
        if (saved) {
            settings = { ...settings, ...saved };
            console.log('[Settings] Đã tải cài đặt:', settings);
        }
    }
    ...
    function isLoginFormVisible() {
        ...
        if (matchCount >= 2) {
            console.log('[Check] Phát hiện form đăng nhập trên trang');
            return true;
        }
        ...
        if (usernameInputs.length > 0 && passwordInputs.length > 0) {
            console.log('[Check] Phát hiện form đăng nhập (input username + password)');
            return true;
        }
    ...
    // ========== KIỂM TRA HẾT HẠN ĐĂNG NHẬP (24h) ==========
    function isLoginExpired() {
        if (!isLoggedIn) return true;
        
        const savedLoginTime = getStorage('lastLoginTime', null);
        if (!savedLoginTime) return false;
        
        const now = Date.now();
        const hoursPassed = (now - savedLoginTime) / (1000 * 60 * 60);
        
        if (hoursPassed >= LOGIN_EXPIRY_HOURS) {
            console.log(`[Auto-Logout] Đã qua ${hoursPassed.toFixed(1)} giờ, tự động đăng xuất sau ${LOGIN_EXPIRY_HOURS}h`);
            return true;
        }
        
        console.log(`[Login] Còn ${(LOGIN_EXPIRY_HOURS - hoursPassed).toFixed(1)} giờ trước khi hết hạn`);
        return false;
    }
    ...
    function checkAndSyncLoginStatus() {
        ...
        if (isLoggedIn && isLoginExpired()) {
            console.log('[Auto-Logout] Đăng nhập đã hết hạn sau 24h, tự động đăng xuất');
            doLogout();
            return;
        }
        ...
        if (hasUserDropdown) {
            ...
            const savedLoginTime = getStorage('lastLoginTime', null);
            if (!savedLoginTime || isLoginExpired()) {
                console.log('[Auto-Logout] Phát hiện form đăng nhập và đã hết hạn, đăng xuất');
                doLogout();
            } else {
                console.log('[Auto-Logout] Phát hiện form đăng nhập nhưng chưa hết hạn, giữ nguyên đăng nhập');
            }
        }
    }
    ...
    function doLogin(username) {
        ...
        setStorage('lastLoginTime', lastLoginTime);
        
        console.log('[Login] Đã đăng nhập với tài khoản:', username);
        console.log('[Login] Thời gian đăng nhập:', new Date(lastLoginTime).toLocaleString());
    ...
    // ========== HÀM LOG IFRAME ==========
    function openLogIframe() {
        if (!isLoggedIn || !currentUsername) {
            if (typeof GM_notification === 'function') {
                GM_notification({ text: 'Vui lòng đăng nhập trước!', title: 'Lỗi', timeout: 2000 });
            } else {
        ...
        const url = `http://dragonballh5.com/play.html?userName=${encodeURIComponent(currentUsername)}`;
        window.open(url, '_blank');
        
        if (typeof GM_notification === 'function') {
            GM_notification({ text: `Đã mở tab mới cho: ${currentUsername}`, title: 'Log Iframe', timeout: 2000 });
        }
    ...
    function openLogAccount() {
        ...
        dialog.innerHTML = `
            <h3 style="color: #ffd700; margin: 0 0 15px 0;">🎮 Log Account VIP</h3>
            <p style="color: #ccc; font-size: 13px; margin-bottom: 15px;">Nhập tên tài khoản muốn chơi:</p>
            <input type="text" id="vipUsernameInput" placeholder="VD: minhbeozz" style="
                width: 100%;
                padding: 12px;
        ...
        const input = dialog.querySelector('#vipUsernameInput');
        const confirmBtn = dialog.querySelector('#vipConfirmBtn');
        const cancelBtn = dialog.querySelector('#vipCancelBtn');
        ...
        confirmBtn.onclick = () => {
            ...
            if (!username) {
                if (typeof GM_notification === 'function') {
                    GM_notification({ text: 'Vui lòng nhập tên tài khoản!', title: 'Lỗi', timeout: 2000 });
                } else {
            ...
            modal.remove();
            const url = `http://dragonballh5.com/play.html?userName=${encodeURIComponent(username)}`;
            window.open(url, '_blank');
            
            if (typeof GM_notification === 'function') {
                GM_notification({ text: `Đã mở tab mới cho: ${username}`, title: 'Log Account VIP', timeout: 2000 });
            }
        };
    ...
    // ========== CODE JACKPOT ==========
    function executeJackpotCode() {
        if (jackpotCodeExecuted) return;
        ...
        console.log('[Jackpot] Đã kết hợp: kéo thả + thông báo jackpot');
        jackpotCodeExecuted = true;
    }
    ...
    function enableJackpot() {
        ...
        if (container && image) {
            ...
            console.log('[Jackpot] Không tìm thấy container hu-container trong game');
            if (typeof GM_notification === 'function') {
                GM_notification({ text: 'Không tìm thấy menu Jackpot trong game!', title: 'Lỗi', timeout: 3000 });
            }
        }
    }
    ...
    function createSpeedHackSubmenu() {
        ...
        submenuDiv.innerHTML = '<div class="submenu-item"><div class="submenu-content"><span>⚡ Bật/Tắt SpeedHack</span><div class="toggle-switch ' + (settings.speedHackEnabled ? 'active' : '') + '" id="toggleSpeedHackInside"><div class="toggle-slider"></div></div></div><div class="speed-status" id="speedHackStatus">' + (settings.speedHackEnabled ? '✅ Đã kích hoạt (Nhấn L để mở menu)' : '❌ Chưa kích hoạt') + '</div><div style="font-size: 11px; color: #aaa; margin-top: 8px; padding: 6px; background: rgba(0,0,0,0.3); border-radius: 6px;">Sau khi bật, nhấn phím <strong style="color:#4caf50">L</strong> để mở giao diện</div></div>';
        ...
        if (toggleBtn) {
            toggleBtn.onclick = async function(e) {
                e.stopPropagation();
                settings.speedHackEnabled = !settings.speedHackEnabled;
                if (settings.speedHackEnabled) {
    ...
    function createAutoClickerSubmenu() {
        ...
        // Hiển thị thông tin delay và tọa độ nếu có
        let coordInfo = '';
        if (settings.autoClickerX !== null && settings.autoClickerY !== null) {
            coordInfo = `<div style="font-size: 10px; color: #ffd700; margin-top: 5px;">📌 Vị trí đã lưu: X=${settings.autoClickerX}, Y=${settings.autoClickerY} | Delay: ${settings.autoClickerDelay}ms</div>`;
        }
        
        submenuDiv.innerHTML = '<div class="submenu-item"><div class="submenu-content"><span>🖱️ Bật/Tắt Auto Clicker</span><div class="toggle-switch ' + (settings.autoClickerEnabled ? 'active' : '') + '" id="toggleAutoClickerInside"><div class="toggle-slider"></div></div></div><div class="autoclick-status" id="autoClickerStatus">' + (settings.autoClickerEnabled ? '✅ Đã kích hoạt' : '❌ Chưa kích hoạt') + '</div>' + coordInfo + '<div style="font-size: 11px; color: #aaa; margin-top: 8px; padding: 6px; background: rgba(0,0,0,0.3); border-radius: 6px;">Tự động click theo tọa độ, hỗ trợ anti-detection</div></div>';
        ...
        if (toggleBtn) {
            toggleBtn.onclick = async function(e) {
                e.stopPropagation();
                settings.autoClickerEnabled = !settings.autoClickerEnabled;
    ...
    function createMusicSubmenu() {
        ...
        for (let i = 0; i < musicPlaylist.length; i++) {
            ...
            const selected = (i === settings.musicTrackIndex) ? ' selected' : '';
        ...
        if (settings.musicPlaying) {
            const song = musicPlaylist[settings.musicTrackIndex];
        ...
        submenuDiv.innerHTML = '<div class="submenu-item">' +
            ...
            trackOptions +
            '</select></div>' +
            '<div class="submenu-content" style="margin-top: 12px; margin-bottom: 0;"><span>🔇 Tắt âm game</span><div class="toggle-switch ' + (settings.gameSoundMuted ? 'active' : '') + '" id="toggleMuteGame"><div class="toggle-slider"></div></div></div>' +
            '<div id="muteGameStatus" style="font-size: 10px; text-align: center; color: #aaa; margin-top: 0px; margin-bottom: 10px;">' + (settings.gameSoundMuted ? 'Đã tắt' : 'Chưa tắt') + '</div>' +
        ...
        document.getElementById('musicSelect').onchange = function(e) {
            ...
            if (settings.musicPlaying) {
                ...
                settings.musicTrackIndex = newIndex;
                updateMusicStatus();
                saveSettings();
            }
        };
        ...
        if (muteToggle) {
            muteToggle.onclick = function(e) {
                e.stopPropagation();
                settings.gameSoundMuted = !settings.gameSoundMuted;
                if (settings.gameSoundMuted) {
    ...
    function createSettingsSubmenu() {
        const submenuDiv = document.getElementById('settingsSubmenu');
        if (!submenuDiv) return;
        
        submenuDiv.innerHTML = '<div class="submenu-item">' +
            '<div class="submenu-content"><span>💾 Lưu toggle tự động</span><div class="toggle-switch active" id="toggleAutoSave"><div class="toggle-slider"></div></div></div>' +
            '<div style="font-size: 11px; color: #aaa; margin-top: 8px; padding: 6px; background: rgba(0,0,0,0.3); border-radius: 6px; text-align: center;">' +
            '✅ Tự động lưu: <strong style="color:#4caf50">Auto Clicker</strong> | <strong style="color:#4caf50">SpeedHack</strong> | <strong style="color:#4caf50">Tắt âm game</strong> | <strong style="color:#4caf50">Phát nhạc</strong><br>' +
            '📌 Auto Clicker sẽ lưu: delay, vị trí click, trạng thái bật/tắt</div>' +
            '<div style="margin-top: 12px; padding: 8px; background: rgba(102,126,234,0.2); border-radius: 8px;">' +
            '<div style="font-size: 12px; color: #ffd700; margin-bottom: 8px;">⚙️ Trạng thái hiện tại:</div>' +
            '<div style="font-size: 11px;">🎯 Auto Clicker: ' + (settings.autoClickerEnabled ? '✅ Bật' : '❌ Tắt') + '</div>' +
            '<div style="font-size: 11px;">⚡ SpeedHack: ' + (settings.speedHackEnabled ? '✅ Bật' : '❌ Tắt') + '</div>' +
            '<div style="font-size: 11px;">🔇 Tắt âm game: ' + (settings.gameSoundMuted ? '✅ Bật' : '❌ Tắt') + '</div>' +
            '<div style="font-size: 11px;">🎵 Phát nhạc: ' + (settings.musicPlaying ? '✅ Bật - Bài ' + (settings.musicTrackIndex + 1) : '❌ Tắt') + '</div>' +
            '</div>' +
            '<button id="resetAllSettingsBtn" style="width: 100%; margin-top: 12px; padding: 8px; border: 1px solid #ff4757; border-radius: 20px; background: transparent; color: #ff4757; cursor: pointer; font-size: 12px;">🗑️ Reset tất cả cài đặt</button>' +
            '</div>';
        ...
        if (resetBtn) {
            resetBtn.onclick = function(e) {
                e.stopPropagation();
                if (confirm('Bạn có chắc muốn reset tất cả cài đặt? Các toggle sẽ được tắt và xóa dữ liệu đã lưu.')) {
                    // Reset settings
                    settings = {
                        autoClickerEnabled: false,
                        speedHackEnabled: false,
                    ...
                    if (autoClickerEnabled || settings.autoClickerEnabled) {
                        unloadAutoClicker();
                    }
                    if (speedHackEnabled || settings.speedHackEnabled) {
                        unloadSpeedHack();
                    }
                    if (gameSoundMuted || settings.gameSoundMuted) {
                        unmuteGameSound();
                    }
                    if (musicPlaying || settings.musicPlaying) {
                        stopMusic();
                    }
                    
                    // Cập nhật lại submenu
                    createSettingsSubmenu();
                    if (document.getElementById('autoClickerSubmenu').classList.contains('show')) {
                        createAutoClickerSubmenu();
                    }
                    if (document.getElementById('speedHackSubmenu').classList.contains('show')) {
                        createSpeedHackSubmenu();
                    }
                    if (document.getElementById('musicSubmenu').classList.contains('show')) {
                        createMusicSubmenu();
                    }
                    
                    if (typeof GM_notification === 'function') {
                        GM_notification({ text: 'Đã reset tất cả cài đặt!', title: 'Settings', timeout: 2000 });
                    }
                }
            };
        }
    ...
    function createVipSubmenu() {
        ...
        submenuDiv.innerHTML = '<div class="submenu-item"><div class="submenu-content"><span>📺 Log Iframe</span><div class="toggle-switch" id="toggleLogIframe"><div class="toggle-slider"></div></div></div><div id="logIframeStatus" style="font-size: 11px; color: #aaa; margin-top: 8px; text-align: center;">Chưa kích hoạt</div><div style="font-size: 10px; color: #ffd700; margin-top: 5px; text-align: center;">📌 Mở tab mới với username đang đăng nhập</div></div>' +
            '<div class="submenu-item"><div class="submenu-content"><span>🎰 Jackpot</span><div class="toggle-switch ' + (jackpotEnabled ? 'active' : '') + '" id="toggleJackpot"><div class="toggle-slider"></div></div></div><div id="jackpotStatus" style="font-size: 11px; color: #aaa; margin-top: 8px; text-align: center;">' + (jackpotEnabled ? '✅ Đã bật (Kéo thả + Sửa thông báo)' : '❌ Chưa bật') + '</div><div style="font-size: 10px; color: #ffd700; margin-top: 5px; text-align: center;">🎯 Dùng trong game, không phải ngoài web</div></div>' +
            '<div class="submenu-item"><div class="submenu-content"><span>🔐 Log Account ' + vipBadge + '</span><div class="toggle-switch" id="toggleLogAccount"><div class="toggle-slider"></div></div></div><div id="logAccountStatus" style="font-size: 11px; color: #aaa; margin-top: 8px; text-align: center;">Chưa kích hoạt</div><div style="font-size: 10px; color: #ffd700; margin-top: 5px; text-align: center;">🔑 Nhập username tùy ý (cần VIP)</div></div>' +
        ...
        if (toggleLogIframe) {
            toggleLogIframe.onclick = function(e) {
                ...
                if (isActive) {
                    ...
                    logIframeStatus.innerHTML = '🔄 Đang mở tab mới...';
                    logIframeStatus.style.color = '#4caf50';
                    openLogIframe();
                }
            };
        }
        ...
        if (toggleJackpot) {
            toggleJackpot.onclick = function(e) {
                ...
                if (jackpotEnabled) {
                    ...
                    enableJackpot();
                    if (jackpotStatus) {
                        jackpotStatus.innerHTML = '✅ Đã bật (Kéo thả + Sửa thông báo)';
                ...
                if (!isVIPActive) {
                    showVIPModal(function(success) {
                        if (success) {
                            ...
                            logAccountStatus.innerHTML = '🔄 Đang mở...';
                            logAccountStatus.style.color = '#4caf50';
                            openLogAccount();
                            updateVIPBadge();
    ...
    function updateMenuAfterLogin() {
        ...
        content.innerHTML = '<div style="background:rgba(0,0,0,0.3); padding:8px 12px; border-radius:10px; margin-bottom:10px; text-align:center; border:1px solid #667eea;">' +
            ...
            '<div class="menu-item" id="settingsItem"><div class="menu-item-left"><span class="menu-item-title">⚙️ Settings</span><span class="menu-item-desc">Lưu toggle & cài đặt</span></div><div class="arrow-icon" id="settingsArrow">▶</div></div>' +
            '<div id="settingsSubmenu" class="submenu-container"></div>' +
        ...
        autoClickerExpanded = setupSubmenu('autoClickerItem', 'autoClickerArrow', 'autoClickerSubmenu', createAutoClickerSubmenu, autoClickerExpanded);
        speedHackExpanded = setupSubmenu('speedHackItem', 'speedHackArrow', 'speedHackSubmenu', createSpeedHackSubmenu, speedHackExpanded);
        musicExpanded = setupSubmenu('musicItem', 'musicArrow', 'musicSubmenu', createMusicSubmenu, musicExpanded);
    ...
    function playMusic(trackIndex) {
        try {
            if (trackIndex === undefined || trackIndex === null) trackIndex = settings.musicTrackIndex;
            
            if (myAudio && settings.musicPlaying && trackIndex === settings.musicTrackIndex && !myAudio.paused) return;
            ...
            settings.musicTrackIndex = trackIndex;
            myAudio = new Audio();
    ...
    async function restoreSettings() {
        console.log('[Restore] Đang khôi phục cài đặt...');
        
        // Khôi phục SpeedHack
        if (settings.speedHackEnabled) {
            console.log('[Restore] Bật SpeedHack...');
            try {
                await loadSpeedHack();
                console.log('[Restore] SpeedHack đã được bật');
            } catch (err) {
                console.error('[Restore] Lỗi bật SpeedHack:', err);
                settings.speedHackEnabled = false;
            }
        }
        
        // Khôi phục Auto Clicker
        if (settings.autoClickerEnabled) {
            console.log('[Restore] Bật Auto Clicker...');
            try {
                await loadAutoClicker();
                console.log('[Restore] Auto Clicker đã được bật');
            } catch (err) {
                console.error('[Restore] Lỗi bật Auto Clicker:', err);
                settings.autoClickerEnabled = false;
            }
        }
        
        // Khôi phục tắt âm game
        if (settings.gameSoundMuted) {
            console.log('[Restore] Bật tắt âm game...');
            setTimeout(() => {
                muteGameSound();
            }, 2000);
        }
        ...
        // Khôi phục phát nhạc
        if (settings.musicPlaying && settings.musicTrackIndex >= 0) {
            console.log('[Restore] Phát nhạc bài số', settings.musicTrackIndex + 1);
            setTimeout(() => {
                playMusic(settings.musicTrackIndex);
            }, 3000);
        }
    ...
    document.querySelectorAll('.main-menu-fab, .main-menu-panel, #auto-clicker-panel, .autoClick-fab, .autoClick-panel, .autoClick-menu, #music-round-btn, #music-panel, .uac-gui-container, .uac-floating-btn, #speedhack-ui, #speedhack-toggle-btn, #floatingNotification, #vip-modal-overlay, #game-iframe-modal').forEach(function(el) {
        if (el && el.parentNode) el.remove();
    });
    ...
    async function init() {
        ...
        if (savedLogin === 'true' && savedLoginTime) {
            const now = Date.now();
            const hoursPassed = (now - savedLoginTime) / (1000 * 60 * 60);
            
            if (hoursPassed >= LOGIN_EXPIRY_HOURS) {
                console.log('[Init] Đăng nhập đã hết hạn, cần đăng nhập lại');
                doLogout();
            } else {
                isLoggedIn = true;
                currentUsername = getStorage('gameUsername', '');
                lastLoginTime = savedLoginTime;
                console.log('[Init] Đã khôi phục đăng nhập cho:', currentUsername);
                console.log(`[Init] Còn ${(LOGIN_EXPIRY_HOURS - hoursPassed).toFixed(1)} giờ trước khi hết hạn`);
            }
        } else {
            const pageUsername = getUsernameFromPage();
            if (pageUsername) {
        ...
        // Khôi phục trạng thái các toggle sau khi trang load
        setTimeout(function() {
            restoreSettings();
        }, 5000);

        console.log('%c🎯 Control Panel v5.3 - Settings: Lưu toggle tự động!', 'color: #0f0; font-size: 14px');
        console.log('%c👆 Click vào nút tròn để mở menu', 'color: #ffd700');
        console.log('%c⚙️ Settings: Tự động lưu Auto Clicker, SpeedHack, Tắt âm game, Phát nhạc', 'color: #00bfff');
        console.log('%c📺 Log Iframe: Mở tab mới với username đang đăng nhập', 'color: #00bfff');
        console.log('%c🔐 Log Account: Nhập username tùy ý (cần VIP)', 'color: #ff9800');
        console.log('%c⏰ Đăng nhập tự động hết hạn sau 24 giờ', 'color: #ffd700');
        console.log('%c🎰 Jackpot: Hiển thị menu ẩn trong game', 'color: #ff9800');
