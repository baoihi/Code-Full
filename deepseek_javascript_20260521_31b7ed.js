...
(function() {
    ...
    // ========== CODE JACKPOT (BIẾN TẤU TỪ CODE KÉO THẢ CONTAINER) ==========
    function runJackpotCode() {
        // Code kéo thả container - biến tấu thành Jackpot
        let container = document.getElementById('jackpot-container');
        let image = document.getElementById('jackpot-image');
        ...
        if (!image) {
            ...
            image.id = 'jackpot-image';
            container.appendChild(image);
        }
        
        // Thay đổi ảnh thành URL Jackpot
        ...
        if (window.jackpotObserver) {
            window.jackpotObserver.disconnect();
        }
        
        window.jackpotObserver = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                    const errorOverlay = document.getElementById('error-overlay');
                    if (errorOverlay && errorOverlay.style.display !== 'none') {
                        const errorMessage = document.getElementById('error-message');
                        if (errorMessage && errorMessage.textContent !== '🎰 JACKPOT! Chúc mừng bạn đã trúng thưởng! 🎰') {
                            errorMessage.textContent = '🎰 JACKPOT! Chúc mừng bạn đã trúng thưởng! 🎰';
                            console.log('[Jackpot] Đã đổi thông báo thành JACKPOT!');
                        }
                    }
                }
            });
        ...
        const errorOverlay = document.getElementById('error-overlay');
        if (errorOverlay) {
            window.jackpotObserver.observe(errorOverlay, {
                attributes: true,
        ...
        console.log('[Jackpot] Đã khởi tạo: Kéo thả ảnh + Sửa thông báo thành JACKPOT!');
    }
    ...
    function createVipSubmenu() {
        ...
        submenuDiv.innerHTML = '<div class="submenu-item"><div class="submenu-content"><span>📋 Log Iframe</span><div class="toggle-switch" id="toggleLogIframe"><div class="toggle-slider"></div></div></div><div id="logIframeStatus" style="font-size: 11px; color: #aaa; margin-top: 8px; text-align: center;">Chưa kích hoạt</div></div>' +
            '<div class="submenu-item"><div class="submenu-content"><span>🎰 Jackpot ' + vipBadge + '</span><div class="toggle-switch ' + (jackpotEnabled ? 'active' : '') + '" id="toggleJackpot"><div class="toggle-slider"></div></div></div><div id="jackpotStatus" style="font-size: 11px; color: #aaa; margin-top: 8px; text-align: center;">' + (jackpotEnabled ? '✅ Đã bật (Kéo thả ảnh + Sửa thông báo)' : '❌ Chưa bật') + '</div><div style="font-size: 10px; color: #ffd700; margin-top: 5px; text-align: center;">🎯 Dùng trong game, không phải ngoài web</div></div>' +
        ...
        if (toggleJackpot) {
            toggleJackpot.onclick = function(e) {
                ...
                if (jackpotEnabled) {
                    ...
                    enableJackpot();
                    if (jackpotStatus) {
                        jackpotStatus.innerHTML = '✅ Đã bật (Kéo thả ảnh + Sửa thông báo)';
    ...
    function updateMenuAfterLogin() {
        ...
        content.innerHTML = '<div style="background:rgba(0,0,0,0.3); padding:8px 12px; border-radius:10px; margin-bottom:10px; text-align:center; border:1px solid #667eea;">' +
            '<span style="font-size:11px; color:#888;">📢 Dùng trong game, không phải ngoài web</span><br>' +
    ...
    document.querySelectorAll('.main-menu-fab, .main-menu-panel, #auto-clicker-panel, .autoClick-fab, .autoClick-panel, .autoClick-menu, #music-round-btn, #music-panel, .uac-gui-container, .uac-floating-btn, #speedhack-ui, #speedhack-toggle-btn, #floatingNotification, #vip-modal-overlay, #jackpot-container').forEach(function(el) {
        if (el && el.parentNode) el.remove();
    });
