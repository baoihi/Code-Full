...
// @version      3.4
// @description  Menu điều khiển với Auto Clicker, Music và SpeedHack (cần đăng nhập để mở menu)
// @author       YourName
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function() {
    ...
    // ========== TẠO PANEL MENU CHÍNH (SAU KHI ĐĂNG NHẬP) ==========
    ...
    mainFab.addEventListener('click', () => {
        ...
        if (savedLogin === 'true') {
            isLoggedIn = true;
            // Đã đăng nhập -> hiện panel hoặc tạo mới nếu chưa có
            ...
            showLoginModal((success) => {
                if (success) {
                    // Đăng nhập thành công -> tạo và hiện panel
                    createMainPanel();
                }
            });
        }
    ...
    async function init() {
        ...
        if (savedLogin === 'true') {
            isLoggedIn = true;
            // Tạo panel sẵn nhưng ẩn đi
            createMainPanel();
            if (mainPanel) mainPanel.style.display = 'none';
        }
        
        console.log('%c✅ Control Panel v3.4 đã sẵn sàng!', 'color: #0f0; font-size: 14px');
