...
// @version 4.3
// @description Menu điều khiển - Tự động đăng nhập từ userDropdown + Pop Up
// @author YourName
// @match *://*/*
...
(function() {
    ...
    function getUsernameFromPage() {
        // Cách 1: Lấy từ thẻ a có id="userDropdown"
        ...
        // Cách 2: Tìm a có class "btn btn-primary dropdown-toggle" chứa icon user-circle
    ...
    function doLogin(username) {
        ...
        if (isLoggedIn && currentUsername === username) {
            console.log('[Login] Đã đăng nhập với', username);
            return true;
        }
        ...
        setStorage('hasEverLoggedIn', 'true');
    ...
    function createPopUp() {
        if (document.getElementById('hu-container')) return document.getElementById('hu-container');
        
        const container = document.createElement('div');
        container.id = 'hu-container';
        container.style.cssText = 'display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999; width: 40px; height: 40px; cursor: move;';
        ...
        function changeErrorMessage() {
            ...
            if (errorMessage && errorMessage.textContent !== 'Chúc mừng nick bạn đã bị ban 😇') {
                ...
                console.log('[PopUp] Đã thay đổi thông báo jackpot');
            }
        }
        ...
        const errorOverlay = document.getElementById('error-overlay');
        if (errorOverlay) {
            observer.observe(errorOverlay, { attributes: true, attributeFilter: ['style'] });
            popUpObserver = observer;
        }
        
        document.addEventListener('click', function() { setTimeout(changeErrorMessage, 100); });
    ...
    function enablePopUp() {
        if (!popUpEnabled) {
            ...
            if (container) {
                ...
                popUpEnabled = true;
                console.log('[PopUp] Đã bật Pop Up');
                if (typeof GM_notification === 'function') {
                    GM_notification({ text: 'Pop Up đã được bật!', title: 'Thông báo', timeout: 2000 });
                }
            }
        }
    }
    ...
    function disablePopUp() {
        if (popUpEnabled) {
            ...
            popUpEnabled = false;
            console.log('[PopUp] Đã tắt Pop Up');
            if (typeof GM_notification === 'function') {
                GM_notification({ text: 'Pop Up đã được tắt!', title: 'Thông báo', timeout: 2000 });
            }
        }
    }

    function createPopUpSubmenu() {
        const submenuDiv = document.getElementById('popUpSubmenu');
        if (!submenuDiv) return;
        
        submenuDiv.innerHTML = '<div class="submenu-item">' +
            '<div class="submenu-content">' +
            '<span>🖼️ Pop Up (Kéo thả)</span>' +
            '<div class="toggle-switch ' + (popUpEnabled ? 'active' : '') + '" id="togglePopUp">' +
            '<div class="toggle-slider"></div>' +
            '</div>' +
            '</div>' +
            '<div style="font-size: 11px; color: #aaa; margin-top: 8px; padding: 6px; background: rgba(0,0,0,0.3); border-radius: 6px;">' +
            '🎯 Tạo ảnh kéo thả + Tự động sửa thông báo Jackpot<br>' +
            '📌 Kéo thả ảnh đến vị trí mong muốn<br>' +
            '💬 Khi có thông báo lỗi sẽ tự động đổi thành "Chúc mừng nick bạn đã bị ban 😇"' +
            '</div>' +
            '</div>';
    ...
    function updateMenuAfterLogin() {
        ...
        content.innerHTML = '<div style="background:rgba(0,0,0,0.3); padding:8px 12px; border-radius:10px; margin-bottom:10px; text-align:center; border:1px solid #667eea;">Tài khoản: <span style="color:#667eea; font-weight:bold;">' + username + '</span> ' + statusBadge + '</div>' +
            ...
            '<div class="menu-item" id="popUpItem"><div class="menu-item-left"><span class="menu-item-title">🖼️ Pop Up</span><span class="menu-item-desc">Ảnh kéo thả + Sửa thông báo</span></div><div class="arrow-icon" id="popUpArrow">▶</div></div>' +
            '<div id="popUpSubmenu" class="submenu-container"></div>' +
        ...
        setupSubmenu('popUpItem', 'popUpArrow', 'popUpSubmenu', createPopUpSubmenu, popUpExpanded);
    ...
    async function init() {
        ...
        console.log('%c🎯 Control Panel v4.3 - Đã thêm Pop Up!', 'color: #0f0; font-size: 14px');
        console.log('%c👆 Click vào nút tròn để mở menu', 'color: #ffd700;');
        console.log('%c🖼️ Mục Pop Up: Ảnh kéo thả + Sửa thông báo Jackpot', 'color: #4caf50;');
