...
// @version 3.8
// @description Menu điều khiển với Auto Clicker, Music và SpeedHack - Tự động đăng nhập/xuất theo trạng thái trang
// @author YourName
// @match *://*/*
...
(function() {
    ...
    function getUsernameFromPage() {
        ...
        // Tìm trong div user-info (trường hợp đã đăng nhập)
        const userInfoDiv = document.querySelector('.user-info .box-login');
        if (userInfoDiv && userInfoDiv.innerHTML.includes('class="fun-btn-v3 sm short"') && userInfoDiv.innerHTML.includes('xu.png')) {
            // Đã đăng nhập nhưng không có h3.rs, thử lấy từ các nguồn khác
            const possibleUsername = document.querySelector('.user-info .box-login')?.innerText;
            if (possibleUsername && !possibleUsername.includes('Đăng nhập')) {
                // Trích xuất username từ text
                const match = possibleUsername.match(/[a-zA-Z0-9_]{3,16}/);
                if (match) return match[0];
            }
        }
        
        console.log('[Auto-Login] Không tìm thấy username đăng nhập trên trang');
        return null;
    }

    // ========== KIỂM TRA TRẠNG THÁI ĐĂNG NHẬP CỦA TRANG ==========
    function isPageLoggedIn() {
        // Kiểm tra có thẻ h3.rs không (đã đăng nhập và hiển thị username)
        const usernameElement = document.querySelector('h3.rs');
        if (usernameElement && usernameElement.textContent.trim()) {
            return true;
        }
        
        // Kiểm tra nút đăng xuất hoặc thông tin user
        const logoutBtn = document.querySelector('[onclick*="clickLogOut"], .fun-btn-v3.sm.short:contains("Thoát")');
        if (logoutBtn) return true;
        
        // Kiểm tra có xu hiển thị không
        const xuElement = document.querySelector('.box-login .fun-btn-v3.sm.short img[src*="xu.png"]');
        if (xuElement) return true;
    ...
    function autoLogout() {
        if (isLoggedIn) {
            console.log('[Auto-Logout] Phát hiện đã đăng xuất khỏi trang, tự động đăng xuất khỏi menu');
            isLoggedIn = false;
            currentUsername = '';
            ...
            if (typeof GM_notification === 'function') {
                GM_notification({ text: 'Đã đăng xuất khỏi Control Panel', title: 'Thông báo', timeout: 2000 });
            }
        }
    }
    ...
    function updateMenuNotLoggedIn() {
        ...
        content.innerHTML = '<div style="background:rgba(0,0,0,0.3); padding:20px 12px; border-radius:10px; margin-bottom:10px; text-align:center; border:1px solid #ff4757;">' +
            '<div style="font-size: 48px; margin-bottom: 10px;">🔐</div>' +
            '<div style="color: #ff4757; font-weight: bold; margin-bottom: 15px;">VUI LÒNG ĐĂNG NHẬP TÀI KHOẢN</div>' +
            '<div style="font-size: 12px; color: #aaa;">Vui lòng đăng nhập vào trang web trước khi sử dụng Control Panel</div>' +
            '</div>';
    }
    ...
    document.querySelectorAll('.main-menu-fab, .main-menu-panel, #auto-clicker-panel, .autoClick-fab, .autoClick-panel, .autoClick-menu, #music-round-btn, #music-panel, .uac-gui-container, .uac-floating-btn, #speedhack-ui, #speedhack-toggle-btn, #floatingNotification, #vip-modal-overlay').forEach(function(el) {
    ...
    function checkAndSyncLoginStatus() {
        ...
        const isPageLoggedInFlag = isPageLoggedIn();
        
        if (isPageLoggedInFlag && pageUsername) {
            // Trang đang đăng nhập
            if (!isLoggedIn || currentUsername !== pageUsername) {
                console.log('[Sync] Phát hiện đăng nhập mới, tự động cập nhật');
                autoLogin(pageUsername);
            }
        } else if (!isPageLoggedInFlag && isLoggedIn) {
            // Trang đã đăng xuất
            autoLogout();
        }
    }
    ...
    function startMonitoring() {
        ...
        // Theo dõi thay đổi DOM (cho các trang dùng AJAX)
    ...
    async function init() {
        ...
        if (savedLogin === 'true' && pageUsername) {
            isLoggedIn = true;
            currentUsername = pageUsername;
            setStorage('gameUsername', pageUsername);
        } else if (pageUsername) {
            autoLogin(pageUsername);
        } else {
            isLoggedIn = false;
            currentUsername = '';
        ...
        // Bắt đầu theo dõi trạng thái đăng nhập
        startMonitoring();
        
        console.log('%c🎯 Control Panel v3.8 (Tự động đồng bộ đăng nhập/xuất)!', 'color: #0f0; font-size: 14px');
        console.log('%c👆 Click vào nút tròn để mở menu', 'color: #ffd700;');
        console.log('%c🔑 Tự động đăng nhập/xuất theo trạng thái trang', 'color: #00adb5;');
