...
// @version 4.7
// @description Menu điều khiển - Tự động đăng nhập/xuất + Pop Up (kéo thả + sửa thông báo)
// @author YourName
// @match *://*/*
...
(function() {
    ...
    function isLoginFormVisible() {
        ...
        // Nếu có ít nhất 2 từ khóa, coi như có form đăng nhập
        if (matchCount >= 2) {
            console.log('[Auto-Logout] Phát hiện form đăng nhập trên trang');
            return true;
        }
        
        // Kiểm tra các input trong form
        ...
        const loginButtons = document.querySelectorAll('button[type="submit"], button:contains("Đăng nhập"), a:contains("Đăng nhập")');
    ...
    function checkAndSyncLoginStatus() {
        ...
        if (hasUserDropdown) {
            // Có userDropdown, thử lấy username
            ...
            // Có form đăng nhập nhưng script đang nghĩ là đã đăng nhập -> đăng xuất
            doLogout();
        } else if (!hasUserDropdown && !hasLoginForm && isLoggedIn) {
            // Không có userDropdown cũng không có form đăng nhập, vẫn giữ đăng nhập
            // (trường hợp chuyển sang web khác nhưng chưa đăng xuất)
        }
    }
    ...
    // ========== CODE POP UP (CHÍNH XÁC THEO YÊU CẦU) ==========
    function runPopUpCode() {
        ...
        const errorOverlay = document.getElementById('error-overlay');
        if (errorOverlay) {
            window.popUpObserver.observe(errorOverlay, {
                attributes: true,
        ...
        console.log('[PopUp] Đã khởi tạo: kéo thả + thông báo jackpot');
    }

    function enablePopUp() {
        if (popUpEnabled) return;
        
        // Tạo container và image nếu chưa có
    ...
    function createPopUpSubmenu() {
        ...
        submenuDiv.innerHTML = '<div class="submenu-item">' +
            ...
            '<span>🖼️ Pop Up (Kéo thả + Sửa thông báo)</span>' +
            ...
            '🎯 Tạo ảnh kéo thả (40x40) có thể di chuyển khắp màn hình<br>' +
            '💬 Tự động sửa thông báo lỗi thành "Chúc mừng nick bạn đã bị ban 😇"<br>' +
            '📌 Kéo thả ảnh đến vị trí mong muốn' +
            '</div>' +
            '</div>';
    ...
    function updateMenuAfterLogin() {
        ...
        content.innerHTML = '<div style="background:rgba(0,0,0,0.3); padding:8px 12px; border-radius:10px; margin-bottom:10px; text-align:center; border:1px solid #667eea;">' +
            '<span style="font-size:11px; color:#888;">📢 Dùng trong game hoặc những web có địa chỉ play-game và user</span><br>' +
    ...
    // ========== CÁC HÀM TẠO SUBMENU (Auto Clicker, SpeedHack, Music, VIP) ==========
    // [Giữ nguyên các hàm createSpeedHackSubmenu, createAutoClickerSubmenu, 
    //  createMusicSubmenu, createVipSubmenu từ script gốc - không thay đổi]
    ...
    async function init() {
        ...
        console.log('%c🎯 Control Panel v4.7 - Đã thêm Pop Up & Tự động đăng xuất!', 'color: #0f0; font-size: 14px');
        ...
        console.log('%c🔐 Tự động đăng xuất khi phát hiện form đăng nhập', 'color: #ff9800;');
